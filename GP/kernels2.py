...
class RBFR(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    """Radial-basis function kernel (aka squared-exponential kernel).
    ...
    """
    def __init__(self, length_scale=1.0, length_scale_bounds=(1e-5, 1e5), rho):
        self.length_scale = length_scale
        self.length_scale_bounds = length_scale_bounds
        self.rho = rho
        self.K_pre = None
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if K_pre == None:
            K = (np.eye(K.shape) -1)/rho**2
        else:
            K = np.multiply(K, (K_pre - 1)/rho**2)
